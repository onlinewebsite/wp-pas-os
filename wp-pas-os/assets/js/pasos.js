jQuery(document).ready(function($){
    function pas_os_ajax_fun(){
        jQuery("#pas-os-form").on('submit',function() {
            ...
            $.ajax({
                url: pasos_ajax_obj.ajaxurl,
                dataType : "json",
                data: {
                    'action': 'pas_os_ajax_request',
                    'pas_os_form' : jQuery(this).serialize(),
                },
                success:function(response) {
                    if(response.status){
                        jQuery('.pas-os-result').html(response.content);
                        jQuery('.pas-os-data').html(response.result.data);
                        pas_os_chart_js();
                    } else {
                        jQuery('.pas-os-result').html(response.content);
                    }
    ...
    jQuery("#pas-os-client").on('submit',function() {
        ...
        $.ajax({
            ...
            dataType : "json",
            data: {
                'action': 'pas_os_ajax_client_request',
                'pas_os_form' : jQuery(this).serialize(),
            },
            success:function(response) {
                if(response.status){
                    jQuery('.pas-os-result').html(response.content);
                    jQuery('.pas-os-data').html(response.form);
                    pas_os_ajax_fun();
                } else {
                    jQuery('.pas-os-result').html(response.content);
                }
